...
export function ProjectCard({ project, featured = false }: ProjectCardProps) {
  ...
  if (featured) {
    return (
      <Link href={`/project/${project.id}`} className="block">
        <div className="venice-card overflow-hidden group hover:scale-[1.02] transition-all duration-300 cursor-pointer">
  ...
  return (
    <Link href={`/project/${project.id}`} className="block">
      <div className="venice-card overflow-hidden group hover:scale-[1.01] transition-all duration-300 cursor-pointer">
        <div className="relative">
          <img
            src={project.imageUrl}
            alt={project.name}
